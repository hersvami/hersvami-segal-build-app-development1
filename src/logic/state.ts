...
export function createProject(
  ...
  return {
    ...
    uiState: { ...state.uiState, view: 'project', showProjectForm: false, showWelcomeEmail: true },
  };
}

export function selectProject(state: AppState, projectId: string): AppState {
  return { ...state, activeProjectId: projectId, uiState: { ...state.uiState, view: 'project' } };
}
...
export function deleteProject(state: AppState, projectId: string): AppState {
  ...
  return {
    ...
    uiState: { ...state.uiState, view: newProjects.length ? 'project' : 'welcome' },
  };
}
...
export function updateVariation(state: AppState, projectId: string, variation: Variation): AppState {
  ...
  return {
    ...
    variations: {
      ...state.variations,
      [projectId]: existing.map(v => v.id === variation.id ? variation : v),
    },
  };
}
...
export function setVariationStatus(state: AppState, projectId: string, variationId: string, status: Variation['status']): AppState {
  ...
  return {
    ...
    variations: {
      ...state.variations,
      [projectId]: existing.map(v => v.id === variationId ? { ...v, status } : v),
    },
  };
}
...
export function hasApprovedQuote(state: AppState, projectId: string): boolean {
  ...
  return variations.some(v => v.documentType === 'quote' && v.status === 'approved');
}

export function getNextVariationNumber(state: AppState, projectId: string): string {
  const variations = state.variations[projectId] || [];
  const count = variations.filter(v => v.documentType === 'variation').length;
