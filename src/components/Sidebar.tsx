      ...
      <div className="flex-1 overflow-y-auto p-4">
        ...
        {projects.map(p => (
          <div key={p.id} onClick={() => onSelectProject(p.id)}
            ...
            <button onClick={e => { e.stopPropagation(); onExportProject(p.id); }} className="p-1 hover:text-white" title="Export">
              <Download className="w-3 h-3" />
            </button>
            <button onClick={e => { e.stopPropagation(); onDeleteProject(p.id); }} className="p-1 hover:text-red-400" title="Delete">
            ...
            <div className="mt-2">
              ...
              {variations.map(v => (
                <button key={v.id} onClick={() => {
                  ...
                  // trigger report view externally
                  ...
                  <span className="font-mono mr-1">{v.documentType === 'quote' ? 'QTE' : 'VAR'}</span>
                  {v.title}
                </button>
              ))}
            </div>
