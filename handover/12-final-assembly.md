...
**What you're building:** Firebase Hosting config with no-cache headers, project linking.

**Verify:** `firebase deploy --only hosting` succeeds. App loads at https://segal-build-app.web.app
...
| Check | How |
|---|---|
| App builds | `npm run build` — no errors |
| Single file output | `dist/index.html` exists and contains all JS/CSS inline |
| Company switching | Toggle Segal Build / Segval in sidebar |
| Project creation | Create project with hero photo |
| Welcome message | 5 channels work (Gmail, Mail, Copy, WhatsApp, SMS) |
| New Quote | 4-step wizard completes, pricing calculates |
| New Variation | Gated by approved quote, external baseline works |
| Builder View | Sliders work, action log shows, trade breakdown displays |
| Customer View | Letterhead, photos, T&Cs, Approve & Sign |
| Progress Hub | Photo upload, stage tracker |
| AI Polish | ✨ button works (or keyword fallback without API key) |
| All files under 300 lines | No file exceeds 300 lines |

---

## If Something Is Missing
1. Check `handover/09-required-fixes.md` for known gaps
2. Check `handover/08-feedback-log.md` for past decisions
3. Check `handover/11-known-issues.md` for known limitations
4. If you can't find the answer, ask the user
